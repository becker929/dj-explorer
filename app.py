        ...
        def shazam_search_hook(d):
            ...
            if 'tmpfilename' in d:
                ...
                if audio_length - byte_index >= min_segment_bytes:
                    ...
                    if 'track' in result:
                        title = unquote(' '.join(result['track']['urlparams']['{tracktitle}'].split('+')))
                        artist = unquote(' '.join(result['track']['urlparams']['{trackartist}'].split('+')))
                        ...
                        if soundcloud_track:
                            ...
                            '''
                            socketio.emit('track', {'embed': iframe, 'widget_id': context.match_count}, room=request.sid)
                        else:
